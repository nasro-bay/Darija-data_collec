...
channel's uploads newest-first (`scrape_channel`) — all funnel into the
same per-video comment-fetch logic, so a video reached any way is never
scraped twice. Sequential by design (no multiprocessing): kept simple and
easy to reason about network-error handling for, at the cost of raw
scraping throughput -- the pipeline (pipeline.py) is where parallelism is
used, for the CPU-bound cleaning/hashing work.
...
def scrape_channel(
    ...
    *,
    ...
    `capped_at`/resume bookkeeping assumes). Each page's selected videos'
    comments are then fetched sequentially via `scrape_video_comments`.
    QuotaExceededError propagates straight out of this function, uncaught.
    """
    ...
    while True:
        ...
        if not hit_cap:
            ...
            # so a later run re-fetches it and continues from where it
            # stopped instead of skipping videos.
